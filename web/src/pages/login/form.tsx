...
export default function LoginForm() {
  ...
  function afterLoginSuccess(params) {
    if (rememberPassword) { // 记住密码
      setLoginParams(JSON.stringify(params));
    } else {
      removeLoginParams();
    }

    localStorage.setItem('userStatus', 'login');  // 记录登录状态
    ...
    <div className={styles['login-form-wrapper']}>
      ...
      >
        ...
        <Form.Item field="password" rules={[{ required: true, message: t['login.form.password.errMsg'] }]}>
          <Input.Password
            prefix={<IconLock />} placeholder={t['login.form.password.placeholder']} onPressEnter={onSubmitClick}
          />
        </Form.Item>
